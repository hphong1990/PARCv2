# PIFNO training script for Burgers Equation
import torch
...
from LossGeneratorLower import loss_generator, compute_loss
...
model_save_path = './model/checkpoint%i_1pde_1mse.pt' % (MAX_EPOCH)
...
for epoch in range(MAX_EPOCH):
    ...
    for data in tqdm(train_dataloader, total=len(train_dataloader)):
        ...
        output = torch.cat((u0.reshape(u0.shape[0], 1, u0.shape[1], u0.shape[2], u0.shape[3]), 
                            output), dim=1)
        # Loss
        loss = compute_loss(output[:, :, :2, :, :], seq, loss_func, R)
