...
def poisson_block(input_shape = (128,192),n_base_features = 64):
    inputs = keras.Input(shape = (input_shape[0], input_shape[1], n_base_features), dtype = tf.float32)
    inputs_2 = keras.Input(shape = (input_shape[0], input_shape[1], 3), dtype = tf.float32)
    concat = Concatenate(axis = -1)([inputs,inputs_2])
    conv = conv_block_down(concat,
                           feat_dim = n_base_features,
                            reps = 1,
                            kernel_size = 3,
                            mode = 'normal')
    conv_res = resnet_block(conv, n_base_features, kernel_size = 3, reps = 2, pooling = False)
    conv_out = Conv2D(1,1, padding='same')(conv_res)
    poisson = keras.Model([inputs,inputs_2], conv_out)
    return poisson
