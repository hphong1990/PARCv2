...
class PARCv2_burgers(PARCv2):
    ...
    def build_differentiator(self):
        # Model initiation
        feature_extraction = layer.feature_extraction_burgers(input_shape = input_shape, n_channel = 3)
        
        advection = [layer.Advection() for _ in range(2)]
        diffusion = [layer.Diffusion() for _ in range(2)]
        velocity_mapping_and_recon = layer.mapping_and_recon_cnn(input_shape = input_shape, n_base_features = 64, n_mask_channel=4, output_channel=2)
    ...
    def build_integrator(self):
        # Model initiation
        velocity_integrator = layer.integrator_cnn(input_shape = input_shape, n_base_features = 64, n_output=2)

        # Input layer
        velocity_prev = keras.layers.Input(shape = (input_shape[0],input_shape[1], 2), dtype = tf.float32)
        velocity_dot = keras.layers.Input(shape = (input_shape[0],input_shape[1], 2), dtype = tf.float32)
