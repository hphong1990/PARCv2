...
def differentiator_climate(n_state_var=3):
    # Model initiation
    feature_extraction = layer.feature_extraction_climate(input_shape = (64,64), n_channel=n_state_var+2)
    
    mapping_and_recon = []
    mapping_and_recon.append(layer.mapping_and_recon_cnn(input_shape = (64,64),n_mask_channel=2, output_channel=1))
    mapping_and_recon.append(layer.mapping_and_recon_cnn(input_shape = (64,64),n_mask_channel=2, output_channel=1))
    mapping_and_recon.append(layer.mapping_and_recon_cnn(input_shape = (64,64),n_mask_channel=2, output_channel=1))
    
    advection = [layer.Advection() for _ in range(n_state_var+2)]
    diffusion = [layer.Diffusion() for _ in range(n_state_var)]
    
    velocity_mapping_and_recon = layer.mapping_and_recon_cnn(input_shape = (64, 64), n_mask_channel=2, output_channel=2)

    # Main computation graph
    input_tensor = Input(shape=(64 , 64, n_state_var+2), dtype = tf.float32)
    ...
    diffusion_temp = diffusion[0](init_state_var[:, :, :, 0:1])
    ...
    output_tensor = Concatenate(axis=-1)([temp_dot,poten_dot,humid_dot, velocity_dot])
...
def integrator(n_state_var = 3):
    ...
    for _ in range(n_state_var):
        state_integrators.append(layer.integrator_cnn(input_shape = (64,64)))

    velocity_integrator = layer.integrator_cnn(input_shape = (64,64), n_output=2)

    state_var_prev = keras.layers.Input(shape = (64, 64, n_state_var), dtype = tf.float32)
    velocity_prev = keras.layers.Input(shape = (64, 64,2), dtype = tf.float32)
    
    state_var_dot = keras.layers.Input(shape = (64, 64,n_state_var), dtype = tf.float32)
...
class PARCv2(keras.Model):
    def __init__(self, n_state_var, n_time_step, step_size, solver = "rk4", mode = "integrator_training", use_data_driven_int = True, differentiator_backbone = 'em', **kwargs):
        ...
        self.differentiator = differentiator_climate(n_state_var=self.n_state_var)
        self.integrator = integrator()
        self.total_loss_tracker = keras.metrics.Mean(name="total_loss")
        if self.mode == "integrator_training":
            self.differentiator.trainable = False
        elif self.mode == "differentitator_training":
